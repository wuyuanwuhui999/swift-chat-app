...
struct HomePage: View {
    ...
    private var tenantListOverlay: some View {
        if showTenantList {
            Color.black.opacity(0.3)
                .ignoresSafeArea()
                .onTapGesture { showTenantList = false }
    ...
    private func handleCurrentModel(_ models: [ChatModel]) {
        ...
        if let modelId = cachedModelId, let matchedModel = models.first(where: { $0.id == modelId }) {
    ...
    private func loadChatHistory(chatId: String) {
        ...
        // 修复：调用正确的接口方法 - 使用 getChatHistoryByChatId 而不是 getChatHistory
        HTTPClient.shared.getChatHistoryByChatId(chatId: chatId) { result in
            DispatchQueue.main.async {
                ...
                case .success(let histories):
                    ...
                    // 转换历史记录为消息列表
                    self.convertHistoryToMessages(histories)
                    print("✅ 加载会话记录成功，已显示 \(self.messages.count) 条消息")
                case .failure(let error):
