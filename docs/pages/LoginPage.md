...
- **依赖服务**：
  ...
  - `HTTPClient.shared.sendEmailVertifyCode(email:completion:)`
...
| 属性 | 类型 | 规则 |
|---|---|---|
| `isLoginButtonEnabled` | `Bool` | `selectedTab == 0` 时 `!account.isEmpty && password.count >= 6`；否则 `isEmailValid && verificationCode.count == 6` |
...
└─ VStack
   ...
   │  │  │  └─ Button(sendVerificationCode) isSendingCode 时 ProgressView，否则图标 paperplane.fill
   ...
   │  │     文字 .themeGray，透明底，overlay RoundedRectangle(cornerRadius: btnHeight/2) 描边 grayColor
...
- **步骤**：调 `Validators.isValidEmail(email)`（`Utils/Validators.swift`，与注册/忘记密码/个人资料页共用），结果写入 `isEmailValid`
- **失败处理**：无提示文案，只让发送按钮与登录按钮置灰

### `sendVerificationCode()`
- **触发**：邮箱输入框内 `paperplane.fill` 按钮点击（`isSendingCode` 时按钮显示 `ProgressView`）
- **步骤**：
  1. `guard isEmailValid else { return }`（双保险，按钮本身也已 `disabled`）
  2. `isSendingCode = true`
  3. 调 `HTTPClient.shared.sendEmailVertifyCode(email: email)`，回调切主线程后 `isSendingCode = false`
  4. `.success(let response)`：按 `response.isSuccess` 分支——成功则 `alertMessage = response.msg ?? "验证码已发送"` + `startCountdown()`；失败则 `alertMessage = response.msg ?? "发送验证码失败"`。两种情况都 `showAlert = true`
  5. `.failure(let error)`：`alertMessage = error.localizedDescription`，`showAlert = true`
- **失败处理**：弹 `alert`，不启动倒计时，用户可立即重试

> 注意：`sendEmailVertifyCode` 把整个 `BaseResponse<EmptyData>` 原样透传给了页面（不像其它方法会把 FAIL 转成 `.failure`），
> 所以页面侧要自己用 `response.isSuccess` 区分成功/失败文案。
...
- **步骤**：
  ...
  4. `.success(let loginResponse)`：
     1. `appState.updateUserData(loginResponse.userData)`
     2. `appState.updateToken(loginResponse.token)`（内部已 `saveToken` + `isLoggedIn = true`）
     3. `navigateToCompanyPage = true`
  5. `.failure(let error)`：`alertMessage = error.localizedDescription` + `showAlert = true`
...
| 2 | `sendEmailVertifyCode` | POST | `/service/user/sendEmailVertifyCode` | Tab 1 点发送图标 | `docs/api/user.md` §6 |
...
- **UI 处理**：`isSuccess` 为真 → 弹「验证码已发送」+ 启动 60s 倒计时；为假 → 弹「发送验证码失败」（文案优先用后端 `msg`）。
...
| 发送验证码按钮 | 图标 `paperplane.fill`，`Dimens.smallIcon`(15)；`isSendingCode` 时显示 `ProgressView`（tint primaryColor）；可用 `Colors.primaryColor` / 不可用 `Colors.grayColor` |
| 倒计时文字 | `Dimens.normalFont`，`Colors.grayColor` |
| 登录按钮 | 高 `Dimens.btnHeight`(50)，圆角 `Dimens.btnHeight / 2`，可用 `Colors.primaryColor` / 禁用 `Colors.grayColor`，文字 `.themeWhite` |
| 注册按钮 | 高 `Dimens.btnHeight`，透明底，描边 + 文字 `Colors.grayColor`，圆角 `Dimens.btnHeight / 2` |
...
3. 点「登录」→ isLoggingIn = true（按钮文案变「登录中...」、显示 ProgressView、禁用）
...
3. 点 paperplane → POST /service/user/sendEmailVertifyCode { email }
     ...
     → 失败（status = FAIL）时弹「发送验证码失败」，不启动倒计时
     → countdown > 0 期间发送按钮 disabled
4. 输入验证码 → isLoginButtonEnabled = isEmailValid && verificationCode.count == 6
...
- Tab 切换**不清空**另一侧的输入，但会停止并重置 `countdown` / `timer`（`onChange(of: selectedTab)`）。
...
2. **同一 `ZStack` 上挂了三个 `fullScreenCover`**：`RegisterPage` / `ForgetPasswordPage` / `CompanyPage`。SwiftUI 对同视图多个同类 presentation 修饰器支持不可靠，目前靠"三个状态互斥"侥幸正常。新增第四个或出现同时为 `true` 的场景就会失效，建议改成枚举 + `fullScreenCover(item:)`。
3. **`onDisappear` 清理不可靠**：本页是被 `fullScreenCover` 弹出的，`CompanyPage` 再以 `fullScreenCover` 盖在它上面时 `LoginPage` 不一定触发 `onDisappear`，`Timer` 可能继续持有强引用。
