...
- **源码**：`chat/chat/UI/Pages/AddModelPage.swift`（约 313 行）
...
- **依赖模型**：无（不解析 `ChatModel`，成功返回值是 `Int`）；类型枚举用 `Models/ChatModelType.swift`
- **依赖服务**：`HTTPClient.shared.addModel`、`AppState.shared`（`currentCompany` / `getCachedCompanyId()`）、`Utils/Validators.swift`、`@Environment(\.dismiss)`
...
| `modelType` | `String`（`@State`） | `ChatModelType.ollama.rawValue` | 模型类型（必填，Picker 选择，存原始值） |
...
| `modelTypes` / 中文文案 | 由 `ChatModelType`（`Models/ChatModelType.swift`）提供 | 枚举 `ollama` / `online`，`displayName` 给 Picker、`rawValue` 提交后端 |

计算属性 / 辅助方法：

| 名称 | 实现 | 含义 |
|---|---|---|
| `isApiKeyRequired` | `ChatModelType.requiresApiKey(for: modelType)` | 在线模型必须填 API Key（`ollama` 本地模型不需要凭证） |
| `Validators.isValidBaseUrl(_:)` | `URL(string:)` 可解析、`scheme` 为 `http`/`https`、`host` 非空 | 模型地址格式校验（提交时执行，`Utils/Validators.swift`，与编辑页共用） |
...
└─ ScrollView（背景 pageBackgroundColor）
   └─ VStack(spacing: middleMargin)，四周 middleMargin 内边距
      ...
      │   │      展示 type.displayName（ollama→「ollama模型」/ online→「在线模型」），tag 用 type.rawValue
      │   ├─ DividerLine()
...
### `Validators.isValidBaseUrl(_ urlString: String) -> Bool`（`Utils/Validators.swift`）
- **触发**：`handleAddModel` 第 3 步（[UpdateModelPage](UpdateModelPage.md) 同样调用）
- **实现**：`URL(string:)` 能解析，且 `scheme` 小写后为 `http`/`https`，且 `host` 非空；任一不满足返回 `false`。

### `ChatModelType`（`Models/ChatModelType.swift`）
- **用途**：模型类型的单一事实源，两页共用
- **成员**：`allCases` 给 Picker 生成选项；`displayName`（「ollama模型」/「在线模型」）；`requiresApiKey`（仅 `online` 为 true）；静态 `displayName(for:)` / `requiresApiKey(for:)` 接收原始字符串，未知取值分别回退「原样展示」与「不强制」。
...
- **改模型类型选项**：只改 `Models/ChatModelType.swift`（加 case + `displayName` + `requiresApiKey`），新增页与 [UpdateModelPage](UpdateModelPage.md) 自动生效；`ollama` / `online` 是与后端约定的现行口径，后端文档里的 deepseek/tongyi 说明尚未更新。
- **改必填规则**：条件必填统一走计算属性（如 `isApiKeyRequired`），同时驱动 `formRow(isRequired:)` 的红 `*`、占位文案与 `isFormValid`，避免三处不一致。
- **改地址校验规则**：改 `Utils/Validators.swift` 的 `isValidBaseUrl(_:)`，两页共用。
